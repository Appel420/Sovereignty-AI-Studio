"""
sovereignty_ai/rag/qdrant_wrapper.py

QdrantRAG -- Encrypted in-memory RAG store with QuadRatchet protection.
Same interface as ChromaRAG and PineconeRAG.
...
class QdrantRAG:
    """
    Encrypted RAG wrapper with a Qdrant-compatible interface.

    In production, replace _mem_store with a real QdrantClient:
      from qdrant_client import QdrantClient
      client = QdrantClient(path="/local/qdrant/storage")  # fully local
    The QuadRatchet encryption layer is identical.
    """

    def __init__(
        self,
        collection_name: str = "sovereign-qdrant",
