...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ObjectMeta { pub namespace: String, pub object_uuid: String, pub content_hash: Hash, pub version: u64, pub provenance_root: Hash, pub temporal_epoch: u64, pub valid_from: u64, pub valid_until: u64 }
#[derive(Clone, Debug, PartialEq, Eq)]
...
impl MemoryObject {
    pub fn new(namespace: &str, uuid: &str, content: &[u8], version: u64, epoch: u64, provenance: Provenance) -> Self {
    ...
    pub fn next_version(&self, content: &[u8], provenance: Provenance, epoch: u64) -> Self { Self::new(&self.meta.namespace, &self.meta.object_uuid, content, self.meta.version + 1, epoch, provenance) }
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct Capability { id: CapabilityId, subject: SubjectId, object_scope: BTreeSet<ObjectId>, allowed_operations: BTreeSet<Operation>, context_hash: Hash, issued_at: u64, expiration: u64, epoch: u64, policy_hash: PolicyId, revoked: bool }
impl Capability {
    ...
    fn valid_at(&self, req: &AccessRequest, now: u64, policy_hash: PolicyId, epoch: u64) -> bool {
        !self.revoked && self.subject == req.subject && self.object_scope.contains(&req.object_id) && self.allowed_operations.contains(&req.operation) && self.context_hash == req.context_hash && self.issued_at <= now && now < self.expiration && self.epoch == epoch && self.policy_hash == policy_hash
    }
}
#[derive(Clone, Debug, PartialEq, Eq)]
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct MemoryWindow { pub window_id: Hash, pub object_ids: BTreeSet<ObjectId>, pub maximum_bytes: u64, pub context_hash: Hash, capability_id: CapabilityId, pub expiration: u64 }
...
impl Core {
    ...
    pub fn revoke_capability(&mut self, subject: SubjectId, capability_id: CapabilityId) -> Result<(), AccessError> { let cap = self.capabilities.get_mut(&capability_id).ok_or(AccessError::InvalidCapability)?; if cap.subject != subject { return Err(AccessError::AuthorityDenied); } cap.revoked = true; Ok(()) }
    pub fn access(&self, req: &AccessRequest, capability_id: CapabilityId, now: u64) -> Result<Decision, AccessError> {
        ...
        if req.operation == Operation::Promote && !matches!(object.authority_state, AuthorityState::Reviewed | AuthorityState::PolicyApproved | AuthorityState::ModelGenerated | AuthorityState::Unverified) { return Err(AccessError::AuthorityDenied); }
        Ok(Decision::Allow)
    }
    ...
    pub fn window_read(&self, subject: SubjectId, window: &MemoryWindow, id: ObjectId, bytes: u64, now: u64) -> Result<Decision, AccessError> {
        ...
        let req = AccessRequest { subject, object_id: id, operation: Operation::Read, context_hash: window.context_hash, epoch: self.epoch }; self.access(&req, window.capability_id, now)
    }
    pub fn transition(&mut self, subject: SubjectId, id: ObjectId, to: AuthorityState, evidence_hash: Hash, authorization_proof: Hash, now: u64, capability_id: CapabilityId) -> Result<ObjectId, AccessError> {
        ...
        let from = current.authority_state;
        let allowed = matches!((from, to), (AuthorityState::ModelGenerated, AuthorityState::Unverified) | (AuthorityState::Unverified, AuthorityState::Reviewed) | (AuthorityState::Reviewed, AuthorityState::PolicyApproved) | (AuthorityState::PolicyApproved, AuthorityState::Authoritative));
        if !allowed { return Err(AccessError::InvalidTransition); }
        let req = AccessRequest { subject, object_id: id, operation: Operation::Promote, context_hash: object_context(&current), epoch: self.epoch };
        self.access(&req, capability_id, now)?;
        let provenance = Provenance { parent_object_ids: vec![id], evidence_hash, source_system: "diamond-core:authority-transition".into() };
        let mut next = current.next_version(&current.payload, provenance, self.epoch);
        next.authority_state = to;
        let destination = next.id;
        self.insert(next)?;
        ...
        self.scar.push(ScarEvent { transition: TransitionRecord { transition_id, subject, policy_id: self.policy_hash, source_object: id, destination_object: destination, timestamp: now, previous_state: from, new_state: to, evidence_hash, authorization_proof } });
        Ok(destination)
    }
