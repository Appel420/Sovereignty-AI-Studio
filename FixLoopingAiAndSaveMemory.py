...
SOVEREIGN_API_URL = os.environ.get("SOVEREIGN_API_URL", "http://localhost:9899/api/ai").rstrip("/")
...
def _chat_sovereign_sync(messages: list[dict], agent: str, authorization: SessionAuthorization) -> str:
    ...
    text = data.get("text") or data.get("response") or data.get("choices", [{}])[0].get("message", {}).get("content", "")
    if not isinstance(text, str) or not text.strip():
        raise RuntimeError("sovereign AI returned no response")
    return text


async def _chat_sovereign(messages: list[dict], agent: str, authorization: SessionAuthorization) -> str:
    return await asyncio.get_running_loop().run_in_executor(None, _chat_sovereign_sync, messages, agent, authorization)


class AIAssistant:
    """Sovereign AI assistant with loop detection and authorized memory."""

    def __init__(self, session_proof: str, *, session: str | None = None, agent: str = "sovereign") -> None:
    ...
    async def init(self) -> None:
        if not self._store or not self._hydrator:
            raise RuntimeError("MemoryStore/MemoryHydrator unavailable; startup denied")
        try:
    ...
    async def process_user_input(self, user_input: str) -> str:
        ...
        ai_response = await _chat_sovereign(self._build_messages(user_input), self.agent, self.authorization)
        if self._detect_loop(ai_response):
            ...
            ai_response = await _chat_sovereign(
                self._build_messages(user_input, reframe=True, previous_response=ai_response),
                self.agent, self.authorization,
            )
            if self._detect_loop(ai_response):
    ...
    def _update_context_summary(self) -> None:
        ...
        self.context_summary = (
            ...
            if recent else "(No prior conversation in this session.)"
        )

    def _build_messages(self, user_input: str, *, reframe: bool = False, previous_response: str = "") -> list[dict[str, str]]:
        system_parts = [
            "You are a sovereign AI assistant running entirely on self-hosted infrastructure. No data leaves the network. Be precise, production-ready, and never fabricate data.",
            self.context_summary,
            f"Authorized identity: {self.authorization.identity_id}",
            f"Authorized runtime mode: {self.authorization.mode}",
        ]
        if reframe:
            system_parts.extend(["IMPORTANT: Your previous response was repetitive. Approach this from a completely different angle.", f"Previous attempt: {previous_response[:REFRAME_PREVIEW_LENGTH]}"])
        return [{"role": "system", "content": "\n\n".join(system_parts).strip()}, {"role": "user", "content": user_input}]

    def _detect_loop(self, response: str) -> bool:
        return any(difflib.SequenceMatcher(None, previous, response).ratio() >= LOOP_SIMILARITY_THRESHOLD for _, previous in self._history[-LOOP_LOOKBACK:])
