...
def test_decode_token_invalid_token(oid_with_credentials: tuple[KeycloakOpenID, str, str]) -> None:
    ...
    decoded_access_token = oid.decode_token(token=***REDACTED***)

    key = oid.public_key()
    key = "-----BEGIN PUBLIC KEY-----\n" + key + "\n-----END PUBLIC KEY-----"
    key = jwcrypto.jwk.JWK.from_pem(key.encode("utf-8"))

    invalid_access_token = ***REDACTED*** + "a"
    with pytest.raises(jwcrypto.jws.InvalidJWSSignature):
        decoded_invalid_access_token = oid.decode_token(token=***REDACTED***, validate=True)
    ...
    decoded_invalid_access_token = oid.decode_token(token=***REDACTED***, validate=False)
...
async def test_a_decode_token_invalid_token(
    ...
    token = await oid.a_token(username=username, password=password)
    access_token = token["access_token"]
    decoded_access_token = await oid.a_decode_token(token=***REDACTED***)
