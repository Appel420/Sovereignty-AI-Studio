...
# Get Token
token = ***REDACTED***.token("user", "password")
token = ***REDACTED***.token("user", "password", totp="012345")

# Get token using Token Exchange
token = ***REDACTED***.exchange_token(token['access_token'], "my_client", "other_client", "some_user")

# Get Userinfo
userinfo = keycloak_openid.userinfo(token['access_token'])

# Refresh token
token = ***REDACTED***.refresh_token(token['refresh_token'])
