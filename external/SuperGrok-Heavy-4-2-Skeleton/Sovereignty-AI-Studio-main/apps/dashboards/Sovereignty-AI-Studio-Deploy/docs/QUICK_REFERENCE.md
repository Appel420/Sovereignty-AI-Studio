...
```bash
curl "http://localhost:9899/api/v1/alerts/?unread_only=true&limit=10"
```

#### WebSocket (JavaScript)
```javascript
const ws = new WebSocket('ws://localhost:9899/api/v1/alerts/ws/1');
...
- **API Docs**: `http://localhost:9899/docs` (when running)
