...
def generate_fresh_profile(
    *,
    ...
    This function generates metadata only. It does not activate the
    deployment, grant authorization, create credentials, or emit SCAR.
    """
    ...
    return DeploymentProfile(
        ...
        identity=IdentitySection(
            owner_ref=owner_ref,
            device_id=_new_id("dev_"),
            authority_id=_new_id("auth_"),
        ),
        vault=VaultSection(
            state_path=str(state_path),
            memory_scope="instance-local",
            evidence_path=str(evidence_path),
        ),
        topology=TopologySection(
            production_branch=production_branch,
            integration_branch=integration_branch,
            development_lanes=list(development_lanes or []),
...
def _minimal_schema_errors(value: Any, schema: dict[str, Any], path: str = "$") -> list[str]:
    """Offline/minimal JSON-Schema validator. Fails closed for unsupported shapes."""
    ...
    if isinstance(value, str):
        ...
        if minimum is not None:
            try:
                ...
                return [f"{path}: invalid minLength in schema"]
            if len(value) < minimum:
                errors.append(f"{path}: shorter than minimum length {minimum}")
        if schema.get("format") == "date-time":
            try:
                ...
                if parsed.tzinfo is None:
                    errors.append(f"{path}: date-time must include timezone information")
    ...
    if isinstance(value, list):
        if schema.get("uniqueItems") is True:
            try:
                ...
                return [f"{path}: cannot evaluate uniqueItems: {exc}"]
...
def _validate_schema_contract(instance: dict[str, Any], schema: Any) -> list[str]:
    if not isinstance(schema, dict):
        return ["schema invalid: schema must be an object"]
...
def validate_profile(profile: DeploymentProfile, schema_path: Path) -> list[str]:
    ...
    An empty list means the profile satisfies the automated contract.
    It does not grant owner approval and does not activate the deployment.
    """
    errors: list[str] = []
    if not isinstance(profile, DeploymentProfile):
        return ["profile must be a DeploymentProfile"]
    ...
    if profile.deployment.schema_version != SCHEMA_VERSION:
        errors.append(f"deployment.schema_version must equal {SCHEMA_VERSION!r}")
    ...
    if not isinstance(profile.identity.key_refs, list):
        errors.append("identity.key_refs must be a list")
    else:
        if len(profile.identity.key_refs) != len(set(profile.identity.key_refs)):
            errors.append("identity.key_refs contains duplicates")
    ...
    try:
        state_path = _normalise_path(vault.state_path)
        evidence_path = _normalise_path(vault.evidence_path)
    except (OSError, RuntimeError, ValueError) as exc:
        errors.append(f"vault path normalization failed: {exc}")
    else:
        if state_path == evidence_path:
            errors.append("state_path and evidence_path collide")
        if state_path == Path("/") or evidence_path == Path("/"):
            errors.append("vault paths must not resolve to filesystem root")

    topology = profile.topology
    if not topology.production_branch:
        errors.append("production branch must be explicit")
    if not topology.integration_branch:
        errors.append("integration branch must be explicit")
    for name, values in (
        ...
        if not isinstance(values, list):
            errors.append(f"topology.{name} must be a list")
            continue
...
def run_validation_pipeline(profile: DeploymentProfile, schema_path: Path) -> ValidationReport:
    """Run automated checks without granting authorization."""
...
def save_profile(profile: DeploymentProfile, path: str | Path) -> None:
    """Atomically persist a validated profile.

    Persistence is not activation and does not grant authorization.
    """
    if not isinstance(profile, DeploymentProfile):
        raise ValidationError("profile must be a DeploymentProfile")
    destination = Path(path).expanduser()
    if destination.name in {"", ".", ".."}:
        raise ValidationError("profile destination must be a file path")
