...
def _check_git_boundary(root: Path) -> CheckResult:
    ...
    if not ok:
        # Contract fixtures may intentionally be plain temporary directories.
        # Git metadata is observational, not required for the read-only checks.
...
def _check_required_files(root: Path) -> CheckResult:
    ...
    return (CheckResult("required-files", False, "missing: " + ", ".join(missing))
            if missing else CheckResult("required-files", True, f"verified={len(REQUIRED_FILES)}"))
...
def _check_python_syntax(root: Path) -> CheckResult:
    ...
    return (CheckResult("python-syntax", False, "; ".join(failures))
            if failures else CheckResult("python-syntax", True, f"parsed={len(PYTHON_FILES)}"))
...
def _check_json(root: Path) -> CheckResult:
    ...
    return (CheckResult("json-config", False, "; ".join(failures))
            if failures else CheckResult("json-config", True, f"parsed={len(JSON_FILES)}"))
...
def _check_sghv_contract(root: Path) -> CheckResult:
    ...
    required = ("DECLARED", "CONFIGURED", "AVAILABLE", "VERIFIED", "ACTIVE",
    ...
    return (CheckResult("sghv119-contract", False, "missing terms: " + ", ".join(missing))
            if missing else CheckResult("sghv119-contract", True, "runtime state vocabulary present"))
...
def _check_devassist_boundary(root: Path) -> CheckResult:
    ...
    text = path.read_text(encoding="utf-8")
    required = ("does not authorize", "route.decision != \"ALLOW\"",
                "route.route not in self._allowed_routes")
    missing = [term for term in required if term not in text]
    return (CheckResult("devassist-boundary", False, "boundary markers missing: " + ", ".join(missing))
            if missing else CheckResult("devassist-boundary", True, "execution remains ALLOW-gated"))


def run_maintenance(root: Path) -> dict[str, Any]:
    root = root.resolve()
    checks = [_check_git_boundary(root), _check_required_files(root),
              _check_python_syntax(root), _check_json(root),
              _check_sghv_contract(root), _check_devassist_boundary(root)]
    passed = all(check.passed for check in checks)
    ok, sha = _run_git(root, "rev-parse", "HEAD")
    if not ok: sha = "unknown"
    ok, branch = _run_git(root, "branch", "--show-current")
