 ...
 * Ports: 9899 (unified internal), 9898 (bridge/gateway), 9899 (auth alias)
...
// ─── CONFIG ───────────────────────────────────────────────────────────
const PORT_UNIFIED = parseInt(process.env.PORT_UNIFIED || '9899');
...
const PP_HASHES = {
  ...
  "cyber_cmd": "1286a1352831192ceaf0b420e7a7db46c023d66aeb5d26d4977fd824f1b70245",
...
function makeProxy(port, label) {
  ...
  const srv = http.createServer((req, res) => {
    ...
    req.pipe(p); p.on('error', () => { try { res.writeHead(502); res.end(); } catch(_){} });
  });
  ...
  wsp.on('connection', (ws2, req2) => {
    ...
    tgt.on('open', () => {
      ...
      tgt.on('close', () => { try{ws2.close();}catch(_){} });
      ws2.on('close', () => { try{tgt.close();}catch(_){} });
    });
    tgt.on('error', () => { try{ws2.close();}catch(_){} });
  });
